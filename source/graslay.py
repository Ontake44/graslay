...
class App:
    ...
    def startBossRushGame(self):
        ...
        if gcommon.CustomNormal:
            # カスタムでも通常にしたい場合（デバッグ）
            GameSession.init(gcommon.DIFFICULTY_NORMAL, Settings.playerStock, gcommon.GAMEMODE_NORMAL, "B1", 1)
        else:
            # 通常
            GameSession.init(gcommon.DIFFICULTY_NORMAL, Settings.playerStock, gcommon.GAMEMODE_BOSSRUSH, "B1", 1)
        GameSession.playerStock -= 1
        GameSession.weaponType = Settings.weaponType
        GameSession.multipleCount = Settings.multipleCount
        #self.stage = Settings.startStage
