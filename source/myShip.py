    ...
    def actionButtonInput(self):
        ...
        if pyxel.btn(pyxel.KEY_UP) or pyxel.btn(pyxel.GAMEPAD1_BUTTON_DPAD_UP) or gcommon.isStickUp() or mouseDy == -1:
            ...
            if gcommon.sync_map_y == 1:
                ...
                if self.y > (88 -16) and self.y  <(88 +16):
                    self.y -= 2
                elif self.y >= (2+2):
    ...
    def executeShot(self):
        ...
        if self.weapon == gcommon.WEAPON_ROUND:
            ...
            if gcommon.checkShotKey() or stick2>0.3:
                ...
                if doShot:
                    if stick2>0.3:
                        self.roundAngle = 63 - int(math.atan2(abs(rightY), rightX)/math.pi * 64)
                        # gcommon.debugPrint("angle = " + str(math.atan2(abs(rightY), rightX)))
                        # gcommon.debugPrint("angle = " + str(self.roundAngle))
